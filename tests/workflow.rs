...
async fn test_workflow_linear_execution() {
    ...
    wf.add_step("step1", create_node(|store| async move {
        store.write().await.insert("s1".into(), serde_json::json!(true));
        store
    }));
    
    wf.add_step("step2", create_node(|store| async move {
        store.write().await.insert("s2".into(), serde_json::json!(true));
        store
    }));
...
async fn test_workflow_conditional_branching() {
    ...
    wf.add_step("router", create_node(|store| async move {
        store.write().await.insert("action".into(), serde_json::json!("path_b"));
        store
    }));
    
    wf.add_step("path_a", create_node(|store| async move {
        store.write().await.insert("visited".into(), serde_json::json!("A"));
        store
    }));

    wf.add_step("path_b", create_node(|store| async move {
        store.write().await.insert("visited".into(), serde_json::json!("B"));
        store
    }));
...
async fn test_workflow_params_merge() {
    ...
    assert_eq!(result.get("default_key").and_then(|v| v.as_str()), Some("default_val"));
    assert_eq!(result.get("override_me").and_then(|v| v.as_str()), Some("new_val"));
}
