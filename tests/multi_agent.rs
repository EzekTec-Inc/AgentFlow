...
async fn test_multi_agent_shared_store() {
    ...
    multi.add_agent(create_node(|store| async move {
        store.write().await.insert("agent1".into(), serde_json::json!(true));
        store
    }));

    multi.add_agent(create_node(|store| async move {
        store.write().await.insert("agent2".into(), serde_json::json!(true));
        store
    }));
...
async fn test_multi_agent_namespaced() {
    ...
    multi.add_agent(create_node(|store| async move {
        store.write().await.insert("result".into(), serde_json::json!("data1"));
        store
    }));

    multi.add_agent(create_node(|store| async move {
        store.write().await.insert("result".into(), serde_json::json!("data2"));
        store
    }));
    ...
    let state = result.read().await;
    assert_eq!(state.get("agent_0.result").and_then(|v| v.as_str()), Some("data1"));
    assert_eq!(state.get("agent_1.result").and_then(|v| v.as_str()), Some("data2"));
}
