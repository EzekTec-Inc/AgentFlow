...
async fn main() {
    dotenv().ok();
    fmt()
        .with_env_filter(EnvFilter::new("agentflow=debug,routing=debug"))
        .init();
    ...
    let triage = create_node(|store: SharedStore| {
        Box::pin(async move {
            ...
            let intent = match agent.prompt(&message).await {
                ...
                Err(e) => {
                    eprintln!("[Triage] LLM error: {e}");
                    "general".to_string()
                }
            };

            // Normalise to known intents
            let intent = if intent.contains("tech") {
                "tech_support"
            } else if intent.contains("bill") {
                "billing"
            } else {
                "general"
            };
            ...
            store
                .write()
                .await
                .insert("action".to_string(), Value::String(intent.to_string()));
            store
        })
    });
    ...
    let tech_node = create_node(|store: SharedStore| {
        Box::pin(async move {
            ...
            store
                .write()
                .await
                .insert("response".to_string(), Value::String(reply));
            store
        })
    });
