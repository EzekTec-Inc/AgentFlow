...
async fn main() {
    ...
    let orchestrator_node = create_node(move |store: SharedStore| {
        Box::pin(async move {
            ...
            let main_task = guard.get("main_task").and_then(|v| v.as_str()).unwrap_or("").to_string();
            ...
            guard.insert("sub_task".to_string(), Value::String("Find out the current system date and time.".to_string()));
            drop(guard);
            ...
            let reasoner_node = create_node(|s: SharedStore| {
                Box::pin(async move {
                    ...
                    let task = g.get("sub_task").and_then(|v| v.as_str()).unwrap_or("").to_string();
                    let tool_output = g.get("date_tool_stdout").and_then(|v| v.as_str()).map(|s| s.to_string());
                    ...
                    if let Some(output) = tool_output {
                        ...
                        g.insert("sub_task_result".to_string(), Value::String(format!("The system date is {}.", output.trim())));
            ...
            // ----------------------------------------------------------------
            let mut guard = store.lock().await;
            let sub_result = guard.get("sub_task_result").and_then(|v| v.as_str()).unwrap_or("").to_string();
            ...
            guard.insert("final_report".to_string(), Value::String(format!("Task: {} | Result: {}", main_task, sub_result)));
            drop(guard);
    ...
    {
        let mut guard = store.lock().await;
        guard.insert("main_task".to_string(), Value::String("Generate a daily system report.".to_string()));
    }
    ...
    println!("\n=== Final Store Output ===");
    println!("Final Report: {:?}", guard.get("final_report").and_then(|v| v.as_str()));
}
