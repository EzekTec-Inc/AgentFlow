...
async fn main() {
    ...
    // Guard against runaway loops — e.g. an LLM critic that never approves
    ...
    let generator_node = create_node(|store: SharedStore| {
        Box::pin(async move {
            ...
            guard.insert(
                "action".to_string(),
                Value::String("critique".to_string()),
            );
    ...
    match flow.run_safe(store).await {
        ...
        Err(AgentFlowError::ExecutionLimitExceeded(msg)) => {
            eprintln!("Reflection aborted — step limit exceeded: {}", msg);
        }
