...
const GENERATOR_SYSTEM: &str =
    "You are a technical writer specialising in Rust. \
...
const CRITIC_SYSTEM: &str =
    "You are a senior technical editor. Review the paragraph you receive. \
...
async fn main() {
    dotenv().ok();
    fmt().with_env_filter(EnvFilter::new("agentflow=debug,reflection=debug")).init();
    ...
    let generator = create_node(|store: SharedStore| {
        Box::pin(async move {
            let (topic, feedback, attempt) = {
                ...
                (
                    g.get("topic").and_then(|v| v.as_str()).unwrap_or("").to_string(),
                    g.get("feedback").and_then(|v| v.as_str()).map(|s| s.to_string()),
            ...
            let agent  = client.agent("gpt-4o-mini").preamble(GENERATOR_SYSTEM).build();
    ...
    let critic = create_node(|store: SharedStore| {
        Box::pin(async move {
            let draft = {
                let g = store.read().await;
                g.get("draft").and_then(|v| v.as_str()).unwrap_or("").to_string()
            };
            ...
            if verdict.trim().starts_with("APPROVED") {
                ...
                let fb = verdict.trim().strip_prefix("REVISE:").unwrap_or("").trim().to_string();
    ...
    store.write().await.insert("topic".to_string(), Value::String(topic.to_string()));
    ...
    match flow.run_safe(store).await {
        Ok(s) => {
            ...
            println!("\nCompleted in {} attempt(s).", g.get("attempt").and_then(|v| v.as_u64()).unwrap_or(0));
        }
